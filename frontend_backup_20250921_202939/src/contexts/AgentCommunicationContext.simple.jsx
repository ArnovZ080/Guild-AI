...
export const AgentCommunicationProvider = ({ children }) => {
  ...
  // Simple celebration trigger without importing from EnhancedMicroCelebrations
  const triggerCelebration = useCallback((type, options = {}) => {
    console.log('Celebration triggered:', type, options);
    // Simple console log instead of complex celebration system
  }, []);
  ...
  const connectWebSocket = useCallback((userId = 'default_user', sessionId = null) => {
    ...
    websocket.onopen = () => {
      ...
      setIsConnected(true);
      setWs(websocket);
      triggerCelebration('collaboration', {
