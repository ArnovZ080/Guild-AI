 ...
 * Manages the flow of "not sure" answers from onboarding to chat-based follow-up questions
 * Integrates with orchestrator to initiate appropriate agent actions
 */
...
class OnboardingFollowUpService {
  ...
  /**
   * Initialize the mapping of onboarding questions to follow-up questions and actions
   */
  initializeFollowUpQuestions() {
    return {
      ...
      'benefit_audience': {
        ...
        followUpQuestion: "Would you like to work on who the best audience for your product or service will be?",
        action: {
      ...
      'audience_problems': {
        ...
        followUpQuestion: "Should we do some research to see what the biggest problem is that your audience struggles with?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'audience_size': {
        ...
        action: {
          ...
          description: 'Ask the user questions about their followings on social platforms and email list size'
        }
      },
      ...
      'marketing_budget': {
        ...
        followUpQuestion: "Would you like some help figuring out what budget for your marketing/advertising will yield the best results?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'priority_3months': {
        ...
        followUpQuestion: "Would you like us to help you determine your biggest priority for the next 3 months?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'guild_working_style': {
        ...
        followUpQuestion: "Would you like to work out and delve deeper into how we can benefit you?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'data_storage': {
        ...
        followUpQuestion: "Should we help you set up a local storage space for your business data?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'brand_colors': {
        ...
        followUpQuestion: "Should we help you choose brand colors that align with your personality and industry?",
        action: {
          type: 'orchestrator_initiate',
      ...
      'brand_differentiation': {
        ...
        followUpQuestion: "Would you like us to help identify and articulate what makes you unique?",
        action: {
          type: 'orchestrator_initiate',
  ...
  /**
   * Analyze onboarding answers to identify "not sure" responses
   */
  analyzeOnboardingAnswers(answers) {
    const notSureAnswers = [];
    
    // Check for common "not sure" phrases
  ...
  /**
   * Generate follow-up questions for chat interface
   */
  ...
  /**
   * Calculate priority for follow-up questions
   */
  calculatePriority(questionId) {
    const highPriority = [
      'business_type', 'benefit_audience', 'customer_avatar', 
      'audience_problems', 'priority_3months'
    ];
    
    const mediumPriority = [
      'pricing_strategy', 'marketing_budget', 'brand_voice_tone',
      'brand_differentiation', 'guild_working_style'
    ];
  ...
  /**
   * Store follow-up questions for later use in chat
   */
  ...
  /**
   * Get next follow-up question for chat
   */
  getNextFollowUpQuestion() {
    if (this.pendingFollowUps.length === 0) {
      // Try to load from localStorage
      const stored = localStorage.getItem('guild_pending_followups');
      if (stored) {
        this.pendingFollowUps = JSON.parse(stored);
      }
    }

    // Return highest priority pending question
    const sortedQuestions = this.pendingFollowUps
  ...
  /**
   * Mark follow-up question as completed
   */
  completeFollowUpQuestion(questionId, userResponse) {
    const question = this.pendingFollowUps.find(q => q.id === questionId);
    if (question) {
      question.status = 'completed';
      question.userResponse = userResponse;
      question.completedAt = new Date().toISOString();
      
      this.completedFollowUps.push(question);
      
      // Update localStorage
      localStorage.setItem('guild_pending_followups', JSON.stringify(this.pendingFollowUps));
      localStorage.setItem('guild_completed_followups', JSON.stringify(this.completedFollowUps));
    }
  }

  /**
   * Initiate orchestrator action for follow-up question
   */
  async initiateOrchestratorAction(action, sessionId) {
    try {
      // This would integrate with the orchestrator API
      const response = await fetch('/api/orchestrator/initiate', {
        method: 'POST',
        headers: {
  ...
  /**
   * Process onboarding completion and generate follow-ups
   */
  processOnboardingCompletion(answers) {
    ...
    if (followUpQuestions.length > 0) {
      ...
      // Store onboarding data with follow-up flags
      ...
      return {
        success: true,
        followUpQuestions,
        message: `We've identified ${followUpQuestions.length} areas where we can help you further develop your business strategy. We'll ask about these in our chat to provide more personalized assistance.`
      };
    }
    ...
    return {
      success: true,
      followUpQuestions: [],
      message: 'Great! You\'ve provided comprehensive information. We\'re ready to help you achieve your business goals.'
    };
  }

  /**
   * Get summary of pending and completed follow-ups
   */
  getFollowUpSummary() {
    return {
      pending: this.pendingFollowUps.filter(q => q.status === 'pending').length,
      completed: this.completedFollowUps.length,
      total: this.pendingFollowUps.length + this.completedFollowUps.length
    };
  }

  /**
   * Clear all follow-up data (for testing or reset)
   */
  clearFollowUpData() {
    this.pendingFollowUps = [];
    this.completedFollowUps = [];
    localStorage.removeItem('guild_pending_followups');
    localStorage.removeItem('guild_completed_followups');
  }
}
