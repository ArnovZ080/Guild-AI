...
const AgentActivityStage = () => {
  const [agents, setAgents] = useState([
    {
      ...
      collaboration: null,
      activityLog: [
        { id: 'a1', text: 'Queued topics from backlog', status: 'done', ts: Date.now() - 6*60*1000 },
        { id: 'a2', text: "Researching 'pricing models'", status: 'running', ts: Date.now() - 3*60*1000 }
      ]
    },
    {
      id: 'marketing-1',
      ...
      activityLog: [
        { id: 'm1', text: 'Pulled audience segments', status: 'done', ts: Date.now() - 8*60*1000 },
        { id: 'm2', text: 'Drafting value propositions', status: 'running', ts: Date.now() - 2*60*1000 }
      ]
    },
    {
      id: 'sales-1',
      ...
      activityLog: [
        { id: 's1', text: 'Scored inbound leads', status: 'done', ts: Date.now() - 10*60*1000 },
        { id: 's2', text: 'Prioritizing outreach list', status: 'running', ts: Date.now() - 60*1000 }
      ]
    },
    {
      id: 'content-1',
      ...
      collaboration: null,
      activityLog: [
        { id: 'c1', text: 'Template cache warmed', status: 'done', ts: Date.now() - 20*60*1000 }
      ]
    }
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setAgents(prev => prev.map(agent => {
        ...
        if (newStatus === 'active' || newStatus === 'collaborating') {
          const pool = ['Scraping web sources','Extracting key facts','Compiling doc','Drafting copy','Analyzing cohorts'];
          if (Math.random() < 0.25) {
            const next = pool[Math.floor(Math.random()*pool.length)];
            const log = { id: `log-${Date.now()}`, text: next, status: 'running', ts: Date.now() };
            const trimmed = ([...(updated.activityLog||[]), log]).slice(-12);
            updated.activityLog = trimmed;
          }
        }
        return updated;
      }));
  ...
  // Drive animation tick for moving handoff dots
  useEffect(() => {
    const id = setInterval(() => setTick((t) => (t + 0.03) % 1), 150);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative bg-card rounded-xl border border-border p-8 overflow-visible" style={{ height: '460px' }}>
        {/* Workstation Areas */}
        ...
        {agents
          ...
          .map(agent => {
            ...
            return (
              ...
              >
                <line
                  x1={`calc(${agent.position.x}% + 12px)`}
                  y1={`calc(${agent.position.y}% + 12px)`}
                  x2={`calc(${collaborator.position.x}% + 12px)`}
                  y2={`calc(${collaborator.position.y}% + 12px)`}
                ...
                {(() => {
                  const t = tick; // 0..1
                  const x = agent.position.x + (collaborator.position.x - agent.position.x) * t;
                  const y = agent.position.y + (collaborator.position.y - agent.position.y) * t;
                  return (
                    <circle
                      cx={`calc(${x}% + 12px)`}
                      cy={`calc(${y}% + 12px)`}
        ...
        {agents.map(agent => (
          ...
          >
            {/* Agent Container */}
            <div className="relative group cursor-pointer" onClick={() => setSelected(agent.id)}>
              {/* Energy Aura */}
              <div
              ...
              <div className="relative w-12 h-12 bg-background rounded-full border-2 border-border flex items-center justify-center text-xl shadow-lg">
                ...
                <div
                  ...
                  style={{
                    background: `conic-gradient(#2563eb ${Math.round((agent.energy || 0)*360)}deg, #e5e7eb 0deg)`
                  }}
                />
      ...
      {/* Agent Details Modal (two-column layout with Pause/Resume and activity) */}
      {selected && (() => {
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-6 relative">
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{a.activity}</p>
                </div>
                <button
                  onClick={() => setPaused(prev => { const next = new Set(prev); if (next.has(a.id)) next.delete(a.id); else next.add(a.id); return next; })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div className="bg-gray-50 rounded-lg p-3 text-sm">
                    <div className="font-medium text-gray-800 mb-1">Current Task</div>
                    <div className="text-gray-700 mb-2">{a.activity}</div>
                    <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${Math.round((a.energy || 0.5) * 100)}%` }} /></div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3 text-sm">
                    <div className="font-medium text-gray-800 mb-2">Recent Assets</div>
                    <div className="grid grid-cols-2 gap-2">
                      <a href="#" className="p-2 rounded border hover:bg-gray-100 flex items-center gap-2"><span>🖼️</span><span className="truncate">concept.png</span></a>
                      <a href="#" className="p-2 rounded border hover:bg-gray-100 flex items-center gap-2"><span>📝</span><span className="truncate">brief.md</span></a>
                    </div>
                  </div>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 text-sm max-h-[50vh] overflow-y-auto">
