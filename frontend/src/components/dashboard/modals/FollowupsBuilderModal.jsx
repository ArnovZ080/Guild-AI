          ...
          {rules.map((r, idx) => (
            <div key={r.id} className="border rounded p-3 grid grid-cols-1 md:grid-cols-4 gap-2 items-end">
              <div>
                ...
                <select value={r.when} onChange={e=>setRules(prev=>prev.map((pr,i)=>i===idx?{...pr, when:e.target.value}:pr))} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
              ...
              <div>
                ...
                <select value={r.action} onChange={e=>setRules(prev=>prev.map((pr,i)=>i===idx?{...pr, action:e.target.value}:pr))} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
              ...
              <div>
                ...
                <input value={r.details||''} onChange={e=>setRules(prev=>prev.map((pr,i)=>i===idx?{...pr, details:e.target.value}:pr))} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Variant B / Case study link" />
              </div>
              <div>
                <label className="block text-gray-700 mb-1">Reason (transparency)</label>
                <input value={r.reason||''} onChange={e=>setRules(prev=>prev.map((pr,i)=>i===idx?{...pr, reason:e.target.value}:pr))} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Why this action is chosen" />
              </div>
            </div>
          ))}
