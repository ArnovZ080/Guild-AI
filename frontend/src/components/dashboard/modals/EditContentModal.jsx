...
const EditContentModal = ({ content, onClose, onSave }) => {
  // Basic state initialized with content
  ...
  const [scheduledDate, setScheduledDate] = useState(content.scheduled_date.split('T')[0]);
  const [scheduledTime, setScheduledTime] = useState(content.scheduled_date.split('T')[1].substring(0, 5));
  const [timezone, setTimezone] = useState(content.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone);
  ...
  const [reviewerNotes, setReviewerNotes] = useState(content.reviewer_notes || '');
  const [editorialGuidelines, setEditorialGuidelines] = useState(content.editorial_guidelines || '');
  ...
  useEffect(() => {
    if (contentType !== content.content_type) {
      setMediaFile(null);
      setMediaPreview('');
    }
  ...
  const validateForm = () => {
    ...
    if (['image', 'video'].includes(contentType) && !mediaFile && !content.media) {
      ...
      >
        <div className="p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Editorial Guidelines</label>
              <textarea
                value={editorialGuidelines}
                onChange={(e) => setEditorialGuidelines(e.target.value)}
                rows="3"
                className="w-full p-2 border border-gray-300 rounded-md"
                placeholder="Add editorial guidelines for this content..."
              />
            </div>
