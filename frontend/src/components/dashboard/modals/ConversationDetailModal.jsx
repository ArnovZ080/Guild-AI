        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              <div className={`p-3 rounded-lg ${getTypeStyle(conversation.type)}`}>
                <TypeIcon className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{conversation.subject}</h2>
                <div className="flex items-center space-x-4 mt-1">
                  ...
                  {conversation.duration && (
                    <span className="text-sm text-gray-600">
                      {conversation.duration}min duration
                    </span>
                  )}
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`text-sm font-medium ${getSentimentStyle(conversation.sentiment)}`}>
                          {conversation.sentiment}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-600">Agent:</span>
                        <span className="text-sm font-medium">{conversation.agentType}</span>
                      </div>
              ...
              {conversation.type === 'voice' && conversation.recordingUrl && (
                <div className="bg-white border rounded-lg p-4">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <button 
                      onClick={() => onDownloadRecording && onDownloadRecording(conversation.recordingUrl)}
                ...
                <div className="space-y-3">
                  <div className="bg-white rounded-lg p-3">
                    ...
                    <p className="text-sm text-gray-700">
                      The {conversation.agentType} agent initiated this conversation because 
                      {conversation.type === 'email' && ' the customer showed high engagement with our product demo.'}
                      {conversation.type === 'voice' && ' the customer had a support issue that required immediate attention.'}
                      {conversation.type === 'chat' && ' the customer was browsing pricing pages indicating purchase intent.'}
                      {conversation.type === 'social' && ' the customer engaged with our social media content.'}
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3">
                    <h4 className="font-medium text-gray-900 mb-2">Agent's reasoning for approach:</h4>
                    <p className="text-sm text-gray-700">
                      Based on customer data analysis, the agent determined that a {conversation.type} approach 
                  ...
                  <div className="bg-white rounded-lg p-3">
                    ...
                    <p className="text-sm text-gray-700">
                      {conversation.nextAction || 'The agent is monitoring customer response to determine the next appropriate action.'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              {conversation.nextAction && (
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4">
                  ...
                  <p className="text-gray-700 mb-2">{conversation.nextAction}</p>
                  <p className="text-sm text-gray-600 mb-4">
                    Due: {conversation.nextActionDate?.toLocaleDateString()}
                  </p>
                  <button
                    onClick={() => {
              ...
              <div className="space-y-3">
                ...
                <button 
                  onClick={() => onStar && onStar(conversation)}
                ...
                <button 
                  onClick={() => onArchive && onArchive(conversation)}
