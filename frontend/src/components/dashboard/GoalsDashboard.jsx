    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-3xl w-full p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-xs uppercase text-gray-500">AI Insights</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="p-3 rounded-lg border border-gray-200">
            ...
            <div className="text-sm font-medium text-gray-900">{goal.status}</div>
          </div>
          <div className="p-3 rounded-lg border border-gray-200">
          ...
          <div className="p-3 rounded-lg border border-gray-200">
            ...
            <div className="text-sm font-medium text-gray-900">{/* computed in detail modal; omit here */}</div>
          </div>
        </div>
        <div className="space-y-4">
          <div className="p-4 rounded-lg border border-gray-200">
            <div className="text-sm font-semibold text-gray-900 mb-2">What’s been done and impact</div>
            <div className="text-sm text-gray-700 whitespace-pre-wrap">{insights?.detailed_feedback?.strengths?.join(', ') || 'No strengths available'}</div>
          </div>
          <div className="p-4 rounded-lg border border-gray-200">
            <div className="text-sm font-semibold text-gray-900 mb-2">What’s left and why</div>
            <div className="text-sm text-gray-700 whitespace-pre-wrap">{insights?.detailed_feedback?.areas_for_improvement?.join(', ') || insights?.analysis || 'No analysis available'}</div>
          </div>
          <div className="p-4 rounded-lg border border-gray-200">
            <div className="text-sm font-semibold text-gray-900 mb-2">Estimated time to goal</div>
            <div className="text-sm text-gray-700">{insights?.overall_evaluation?.estimated_time_to_goal || 'Pending estimation'}</div>
          </div>
          <div className="p-4 rounded-lg border border-gray-200">
            <div className="text-xs uppercase text-gray-500 mb-1">Transparency</div>
            <div className="text-xs text-gray-600">These insights are generated by the Judge layer using current goal metrics and progress. Rationales and scoring are included when available.</div>
          </div>
        </div>
      </div>
    </div>
...
const GoalsDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <AddGoalModal isOpen={showAddGoal} onClose={() => { setShowAddGoal(false); setEditPrefill(null); }} onCreated={handleGoalCreated} prefill={editPrefill || undefined} />
      <GoalDetailModal
        goal={selectedGoal}
