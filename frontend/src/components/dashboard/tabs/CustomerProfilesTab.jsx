      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedProfiles.map((profile, index) => (
          ...
          >
            ...
            {/* Health Indicators moved into modal Overview tab */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {(() => {
                      ...
                      const indicators = p.health_indicators || {
                        usage_frequency: p.engagement_score >= 80 ? 'increasing' : p.engagement_score >= 60 ? 'stable' : 'decreasing',
                        feature_adoption_depth: p.total_orders >= 10 ? 'increasing' : p.total_orders >= 5 ? 'stable' : 'decreasing',
                        recent_activity_recency: (() => {
                          const days = Math.max(0, Math.round((Date.now() - new Date(p.last_activity).getTime())/(1000*60*60*24)));
                          return days <= 7 ? 'increasing' : days <= 21 ? 'stable' : 'decreasing';
                        })(),
                        ticket_volume_escalations: p.support_tickets <= 1 ? 'increasing' : p.support_tickets <= 3 ? 'stable' : 'decreasing',
                        sentiment_trend: (p.sentiment_score || 0.7) >= 0.8 ? 'increasing' : (p.sentiment_score || 0.7) >= 0.65 ? 'stable' : 'decreasing'
                      };
                      return (
