            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    { label: 'Retention Rate', value: customer_metrics?.retention_metrics?.retention_rate?.current, unit: '%'},
                    { label: 'Churn Rate', value: customer_metrics?.retention_metrics?.churn_rate?.current, unit: '%'},
                    { label: 'Funnel Conversion', value: customer_metrics?.acquisition_metrics?.funnel_conversion_rate?.current, unit: '%'},
                    { label: 'NPS Score', value: customer_metrics?.satisfaction_metrics?.nps_score?.current, unit: ''}
                  ].map((kpi, idx) => (
                    <div key={idx} className="p-3 bg-gray-50 rounded border flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-gray-900">{kpi.value ?? '—'}{kpi.unit}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    { agent: 'Customer Intelligence Agent', role: 'Analyzed customer metrics and surfaced patterns' },
                    { agent: 'Judge Agent', role: 'Validated insight quality and urgency thresholds' },
                    { agent: 'CRM Automation Agent', role: 'Prepared actionable segments and outreach options' },
                    { agent: 'Orchestrator Agent', role: 'Coordinated data retrieval and scoring' }
                  ].map((row, idx) => (
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="p-3 bg-gray-50 rounded border">
                  ...
                  <p className="text-xs text-gray-600">Customer Intelligence, Judge, CRM Automation, Orchestrator</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    { label: 'Retention Rate', current: customer_metrics?.retention_metrics?.retention_rate?.current, target: 85 },
                    { label: 'Churn Rate', current: customer_metrics?.retention_metrics?.churn_rate?.current, target: 10 },
                    { label: 'Funnel Conversion', current: customer_metrics?.acquisition_metrics?.funnel_conversion_rate?.current, target: 25 }
                  ].map((row, idx) => (
                    <div key={idx} className="p-3 bg-gray-50 rounded border">
                      <p className="text-sm font-medium text-gray-800">{row.label}</p>
                      <p className="text-xs text-gray-600">Current: {row.current ?? '—'} • Target: {row.target}</p>
                    </div>
                  ))}
                </div>
              </div>
