...
const CalendarView = () => {
  ...
  // Real agent integration for Personal Assistant
  const handleSchedulingRequest = async (request) => {
    try {
      setShowWorkflow(true);
      
      // Step 1: Personal Assistant Agent processes the request
      await sendTaskToAgent('personal_assistant_agent', {
      ...
      // Step 2: Calendar Harmony Agent checks for conflicts
      await sendTaskToAgent('calendar_harmony_agent', {
        type: 'conflict_check',
        request: request,
        previous_agent: 'personal_assistant_agent',
        user_id: 'current_user'
      });
      ...
      setAgentWorkflow(prev => [...prev, {
        id: Date.now() + 1,
        step: 'conflict_check',
        agent: 'Calendar Harmony Agent',
        status: 'processing',
        timestamp: new Date(),
        depends_on: 'schedule_request'
      }]);

      // Step 3: Notification Agent sets up reminders
      await sendTaskToAgent('notification_agent', {
        type: 'setup_reminders',
        request: request,
        previous_agents: ['personal_assistant_agent', 'calendar_harmony_agent'],
        user_id: 'current_user'
      });
  ...
  const handleNaturalLanguageSchedule = async (input) => {
    try {
      await handleSchedulingRequest(input);
      setSchedulingInput('');
    } catch (error) {
      console.error('Error processing natural language schedule:', error);
    }
  };
  ...
  // PA Chat functionality with real agents
  const handlePAMessage = async (message) => {
    ...
    try {
      // Send to Personal Assistant Agent
      await handleSchedulingRequest(message);
      
      // Add agent response message
      const agentMessage = {
        id: Date.now() + 1,
        type: 'assistant',
        content: "I've received your request and I'm coordinating with my agent team to help you. You can see the progress above.",
        timestamp: new Date()
      };
      setPaMessages(prev => [...prev, agentMessage]);
    } catch (error) {
      console.error('Error processing PA message:', error);
      const errorMessage = {
        id: Date.now() + 1,
        type: 'assistant',
      ...
      setPaMessages(prev => [...prev, errorMessage]);
    } finally {
      setIsPaTyping(false);
    }
    ...
    <div className="space-y-6">
      ...
      {showWorkflow && (
        <AgentWorkflowVisualizer 
          workflowType="personal_assistant"
