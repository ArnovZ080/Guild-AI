...
const CustomerJourneyConstellation: React.FC = () => {
  ...
  const getModeStyles = () => {
    switch (currentMode) {
      case 'morning':
        return {
          background: 'from-sky-dawn via-blue-900 to-indigo-900',
          text: 'text-sky-dawn'
        };
      case 'active':
        return {
          background: 'from-sky-day via-purple-900 to-black',
          text: 'text-sky-day'
        };
      case 'evening':
        return {
          background: 'from-sky-dusk via-purple-900 to-black',
          text: 'text-sky-dusk'
        };
      default:
        return {
  ...
  const getStarBrightness = (engagement: number) => {
    return 0.4 + (engagement * 0.6);
  };

  return (
    <div className={`relative w-full h-96 bg-gradient-to-b ${modeStyles.background} rounded-lg overflow-hidden shadow-lg border border-gray-200`}>
      {/* Background stars */}
      ...
      {selectedCustomer && (
        <motion.div
          className="absolute top-4 right-4 bg-white rounded-lg p-4 shadow-xl max-w-xs z-10"
      ...
      <div className="absolute top-4 left-4">
        ...
        <p className="text-sm text-white/70">
          Your customers across the journey
        </p>
      </div>

      {/* Constellation connections for advocates */}
      {customers
        .filter(customer => customer.stage === 'advocate')
        .map(advocate => 
          customers
            .filter(customer => customer.stage === 'prospect' || customer.stage === 'lead')
            .map((otherCustomer, index) => (
              <motion.div
                key={`connection-${advocate.id}-${otherCustomer.id}`}
                className="absolute border border-white border-opacity-20"
                style={{
                  left: `${advocate.x}%`,
                  top: `${advocate.y}%`,
                  width: `${Math.sqrt(Math.pow(advocate.x - otherCustomer.x, 2) + Math.pow(advocate.y - otherCustomer.y, 2))}%`,
                  height: '1px',
                  transformOrigin: 'left center',
                  transform: `rotate(${Math.atan2(otherCustomer.y - advocate.y, otherCustomer.x - advocate.x) * 180 / Math.PI}deg)`,
                }}
                animate={{
                  opacity: [0.2, 0.5, 0.2],
                }}
