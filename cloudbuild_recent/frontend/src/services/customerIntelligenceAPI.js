...
// API Service for Customer Intelligence Agent
export class CustomerIntelligenceAPIService {
  ...
  async executeCustomerAction(actionId, actionData = {}) {
    return this.request('/customer/execute-action', {
      method: 'POST',
  ...
  getMockCustomerProfiles(segment, limit) {
    ...
    for (let i = 0; i < Math.min(limit, 50); i++) {
      ...
      profiles.push({
        ...
        source: ['Organic', 'Paid Ads', 'Referral', 'Social'][i % 4],
        last_purchase: new Date(Date.now() - (i % 60) * 24 * 60 * 60 * 1000).toISOString(),
        average_order_value: 150 + (i * 5),
        preferred_communication: ['Email', 'Phone', 'SMS', 'Chat'][i % 4],
        support_preferences: ['Self-service', 'Email', 'Phone', 'Chat'][i % 4],
        notes: `Customer notes for customer ${i}`,
        // Journey data
        journey_stage: ['awareness', 'consideration', 'decision', 'retention', 'advocacy'][i % 5],
        touchpoints: [
          { type: 'email', date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), status: 'opened' },
          { type: 'website', date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), status: 'visited' },
          { type: 'support', date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), status: 'resolved' }
        ],
        // Health indicators
        health_indicators: {
