"""
Enhanced Accounting Agent - Production Ready for Google Cloud Vertex AI
...
class AccountingAgent(AgentTemplate):
    """
    Enhanced Accounting Agent with full BaseAgent integration
    Production-ready for Google Cloud Vertex AI deployment
    """
    
    def __init__(self, agent_id: str = "accounting"):
        # Define specific capabilities
        specific_capabilities = [
            AgentCapability(
                name="budget_optimization",
                description="Budget Optimization management and optimization",
                complexity=TaskComplexity.MODERATE,
                estimated_duration=15
            ),
            AgentCapability(
                name="financial_management",
                description="Financial Management management and optimization",
                complexity=TaskComplexity.MODERATE,
                estimated_duration=15
            ),
            AgentCapability(
                name="financial_reporting",
                description="Financial Reporting management and optimization",
                complexity=TaskComplexity.MODERATE,
                estimated_duration=15
            ),
            AgentCapability(
            ...
            AgentCapability(
                ...
                description="Report Generation management and optimization",
                complexity=TaskComplexity.MODERATE,
                estimated_duration=15
            )
        ]
        ...
        super().__init__(
            ...
            description="Comprehensive financial data processing and reporting using advanced prompting strategies.",
            capabilities=[
                "budget_optimization",
    ...
    def get_required_task_fields(self) -> List[str]:
        """Get list of required fields for finance tasks"""
        return [
            'description',
            'finance_context',
            'objective'
    ...
    async def validate_task_specifics(self, task: Dict[str, Any]) -> bool:
        """Validate finance-specific task requirements"""
    ...
    async def generate_specific_questions(self, task: Dict[str, Any]) -> List[str]:
        """Generate finance-specific clarification questions"""
        questions = []
    ...
    async def _execute_agent_specific_task(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """
        Execute finance-specific task logic
        """
        try:
            logger.info(f"{self.name}: Starting finance task execution")
            
            # Send status update
            await self.send_status_update("processing", 30, f"{self.name} is processing your finance request...")
            
            # Extract task parameters
            objective = task.get('description', f'{self.name} task execution')
            context = task.get(f'finance_context', {})
            
            # Route to appropriate operation based on task type
            task_type = task.get('task_type', 'general').lower()
            
            if task_type == 'analysis':
                result = await self._perform_finance_analysis(task, session_id)
            elif task_type == 'strategy':
                result = await self._develop_finance_strategy(task, session_id)
            elif task_type == 'execution':
                result = await self._execute_finance_operations(task, session_id)
            else:
                result = await self._comprehensive_finance_solution(task, session_id)
            
            return result
            
        except Exception as e:
            logger.error(f"{self.name}: finance task execution failed: {e}")
            return {
                "success": False,
                "error": str(e),
                "operation": "finance_execution"
            }
    
    async def _perform_finance_analysis(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """Perform finance analysis"""
        await self.send_status_update("analyzing", 50, f"{self.name} is performing finance analysis...")
        
        # Build analysis prompt
        prompt = self._build_finance_analysis_prompt(task)
        ...
        return {
            "success": True,
            "operation": "finance_analysis",
            "analysis_results": response,
            "confidence_score": 85.0,
            "analysis_time": datetime.now().isoformat()
        }
    
    async def _develop_finance_strategy(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """Develop finance strategy"""
        await self.send_status_update("strategizing", 60, f"{self.name} is developing finance strategy...")
        
        # Build strategy prompt
        prompt = self._build_finance_strategy_prompt(task)
        ...
        return {
            "success": True,
            "operation": "finance_strategy",
            "strategy_results": response,
            "implementation_plan": self._create_implementation_plan(task),
            "strategy_time": datetime.now().isoformat()
        }
    
    async def _execute_finance_operations(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """Execute finance operations"""
        await self.send_status_update("executing", 70, f"{self.name} is executing finance operations...")
        ...
        return {
            "success": True,
            "operation": "finance_operations",
            "operations_completed": len(operations_results),
            "results": operations_results,
            "execution_time": datetime.now().isoformat()
        }
    
    async def _comprehensive_finance_solution(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """Provide comprehensive finance solution"""
        await self.send_status_update("solving", 80, f"{self.name} is providing comprehensive finance solution...")
        ...
        return {
            "success": True,
            "operation": "comprehensive_finance_solution",
            "solution": solution,
    ...
    def _build_finance_analysis_prompt(self, task: Dict[str, Any]) -> str:
        """Build finance analysis prompt"""
        return f"""
# {self.name} - Finance Analysis

## Task Context
**Objective:** {task.get('description', 'Finance analysis task')}
**Context:** {json.dumps(task.get(f'finance_context', {}), indent=2)}

## Analysis Requirements
Perform comprehensive finance analysis including:
...
"""
    ...
    def _build_finance_strategy_prompt(self, task: Dict[str, Any]) -> str:
        """Build finance strategy prompt"""
        return f"""
# {self.name} - Finance Strategy Development

## Task Context
**Objective:** {task.get('description', 'Finance strategy development')}
**Context:** {json.dumps(task.get(f'finance_context', {}), indent=2)}

## Strategy Requirements
Develop comprehensive finance strategy including:
...
"""
        ...
        return [
            {
                ...
                "recommendation": "Implement core finance processes",
                "priority": "high",
                "impact": "significant"
            },
            {
                "category": "Medium-term Goals",
                "recommendation": "Optimize and scale finance operations",
                "priority": "medium",
                "impact": "moderate"
            },
            {
                "category": "Long-term Vision",
                "recommendation": "Advanced finance automation and AI integration",
                "priority": "low",
    ...
    def _generate_next_steps(self, task: Dict[str, Any]) -> List[str]:
        """Generate next steps"""
        return [
            f"Review and approve finance strategy",
            f"Allocate resources for finance implementation",
            f"Begin Phase 1 of finance execution",
            f"Set up monitoring and tracking for finance metrics",
            f"Schedule regular finance review meetings"
        ]
    
    def can_handle_task(self, task: Dict[str, Any]) -> bool:
        """Enhanced task matching for finance operations"""
    ...
    def estimate_task_complexity(self, task: Dict[str, Any]) -> TaskComplexity:
        """Estimate complexity of finance tasks"""
