...
async def process_chat_orchestration(
    ...
    try:
        ...
        try:
            from guild.src.core.orchestration.enhanced_orchestrator import EnhancedOrchestrator as Orchestrator
...
Respond naturally and helpfully."""
            ...
            try:
                ...
                if smart_response.get('error'):
                    logger.error(f"Gemini API returned error: {smart_response.get('error')}")
                    raise Exception(f"Gemini API error: {smart_response.get('error')}")
                ...
                # Greeting responses
                if any(greeting in lower_objective for greeting in ['hello', 'hi', 'hey', 'how are you', 'how is it going']):
                    response_text = f"Hey {user_name}! I'm doing great, thanks for asking! I'm your AI business orchestrator with access to 115+ specialized agents. I'm here to help you grow your business - whether that's creating content, analyzing your finances, or developing marketing strategies. What would you like to work on today?"
                
                # Content creation requests
                elif any(keyword in lower_objective for keyword in ['content', 'post', 'blog', 'article', 'write']):
                    if 'facebook' in lower_objective:
                    ...
                    elif 'instagram' in lower_objective:
                        response_text = f"Perfect! I can help you create Instagram content that engages your audience. Our Content Strategy and Visual Creation agents can develop posts, stories, and reels that align with your brand. What's your main goal - brand awareness, lead generation, or community building?"
                    else:
                        response_text = f"Great! I can help you create compelling content across all platforms. Our Content Strategy agents can develop blog posts, social media content, email campaigns, and more. What type of content are you looking to create, and which platforms should we focus on?"
                
                # Marketing requests
                elif any(keyword in lower_objective for keyword in ['marketing', 'campaign', 'ads', 'promote']):
                    response_text = f"Excellent! I can help you develop comprehensive marketing strategies. Our Marketing Campaign agents can create targeted campaigns, optimize ad performance, and track ROI. What's your main marketing goal - increasing brand awareness, generating leads, or driving sales?"
                
                # Business strategy requests
                elif any(keyword in lower_objective for keyword in ['strategy', 'plan', 'grow', 'improve', 'help']):
                    response_text = f"I'd love to help you develop a strategic plan for your business! Our Business Intelligence agents can analyze your current situation and create a roadmap for growth. What specific area would you like to focus on - revenue growth, customer acquisition, operational efficiency, or something else?"
                
                # General capability questions
                elif any(keyword in lower_objective for keyword in ['what can you do', 'what do you do', 'capabilities', 'help']):
                    response_text = f"As your Fortune 500-level business orchestrator, I can coordinate 115+ specialized agents to handle everything from content creation and social media marketing to financial analysis and business growth strategies. I can help you with content creation, marketing campaigns, financial planning, customer analysis, and much more. What specific business challenge would you like to tackle first?"
