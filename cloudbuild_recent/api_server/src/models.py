...
class Conversation(Base):
    """Chat conversation threads for users"""
...
class ConversationMessage(Base):
    ...
    # Message metadata
    model_used = Column(String(100), nullable=True)  # gemini-1.5-flash, gpt-4, etc.
    tokens_used = Column(Integer, nullable=True)
    processing_time = Column(Float, nullable=True)  # seconds
    
    # Workflow integration
    workflow_id = Column(String(50), nullable=True)  # If message triggered a workflow
    agent_responses = Column(JSON, default=lambda: [])  # Array of agent responses
