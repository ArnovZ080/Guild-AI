...
class GeminiProvider:
    ...
    def __init__(self):
        ...
        try:
            # Check if we're in Cloud Run (has GOOGLE_CLOUD_PROJECT)
            if os.getenv('GOOGLE_CLOUD_PROJECT'):
                # In Cloud Run, use default credentials
                vertexai.init(project=self.project_id, location=self.location)
                logger.info(f"✅ Vertex AI initialized in Cloud Run: {self.project_id} in {self.location}")
            else:
                # Local development - try to use application default credentials
                vertexai.init(project=self.project_id, location=self.location)
                logger.info(f"✅ Vertex AI initialized locally: {self.project_id} in {self.location}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Vertex AI connection test failed: {test_error}")
                # Still mark as initialized if the init worked
                
        except Exception as e:
            logger.error(f"❌ Failed to initialize Vertex AI: {e}")
            logger.error(f"Project: {self.project_id}, Location: {self.location}")
            logger.error(f"Environment: GOOGLE_CLOUD_PROJECT={os.getenv('GOOGLE_CLOUD_PROJECT')}")
            logger.error(f"Make sure the service account has Vertex AI permissions")
