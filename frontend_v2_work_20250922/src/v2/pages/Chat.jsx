...
const ChatPageV2 = ({ onNavigateToDashboard, onNavigateToMarketplace, onNavigateToCalendar, onNavigateToGoals, onNavigateToAchievements, onNavigateToGrowth, onNavigateToCustomers, onNavigateToConversations, onNavigateToConnectors }) => {
  ...
  const onboardingQuestionMap = {
    "Who do you imagine benefits the most from what you offer?": { 
      label: "Work on finding your ideal audience.", 
      action: "find_ideal_audience_action" 
    },
    "Do you already have a customer avatar (ideal client profile)?": { 
      label: "Build your Ideal Customer Avatar.", 
      action: "build_ideal_customer_avatar_action" 
    },
    "What's the biggest problem your audience struggles with?": { 
      label: "Research your audience's biggest problems.", 
      action: "research_audience_problems_action" 
    },
    "How big is your current audience or customer base?": { 
      label: "Determine the size of your current audience.", 
      action: "determine_audience_size_action" 
    },
    "What type of business are you running (or planning to run)?": { 
      label: "Find the best business for you.", 
      action: "find_best_business_fit_action" 
    },
    "How are you handling pricing right now?": { 
      label: "Work on your pricing strategy.", 
      action: "work_on_pricing_strategy_action" 
    },
    "Do you have a monthly marketing/advertising budget?": { 
      label: "Figure out the best marketing/advertising budget.", 
      action: "figure_out_marketing_budget_action" 
    },
    "What's your #1 priority for the next 3 months?": { 
      label: "Determine your biggest priority for the next 3 months.", 
      action: "determine_3_month_priority_action" 
    },
    "How do you prefer Guild to work with you?": { 
      label: "Delve deeper into how Guild can benefit you.", 
      action: "explore_guild_benefits_action" 
    },
    "Where would you prefer to store your business data?": { 
      label: "Set up local storage for your business data.", 
      action: "setup_local_storage_action" 
    },
    "How do you want Guild to handle sensitive information?": { 
      label: "Work on a secure storage solution for sensitive data.", 
      action: "work_on_secure_storage_action" 
    },
  };
  ...
  const getSuggestedActions = () => {
    ...
    const incompleteQuestions = Object.keys(onboardingQuestionMap).filter(q => 
      onboardingData[q.replace(/\s+/g, '-').toLowerCase()]?.toLowerCase()?.includes('not sure') || 
      onboardingData[q.replace(/\s+/g, '-').toLowerCase()]?.toLowerCase()?.includes('don\'t know')
    );

    return incompleteQuestions.map(q => ({
      ...onboardingQuestionMap[q],
      icon: '🧠' 
    }));
  };
